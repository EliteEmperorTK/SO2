...
// Escribimos en un fichero. Devuelve la cantidad de Bytes escritos
int mi_write_f(unsigned int ninodo, const void *buf_original, unsigned int offset, unsigned int nbytes)
{
    ...
    { // Caso 1 (Escritura en 1 solo bloque)
        ...
        bwrite(nbfisico, buf_bloque);                     // Escribimos lo del buffer donde toca

        cant_bytes = nbytes;

        // cant_bytes = bwrite(nbfisico, buf_bloque); // Escribimos lo del buffer donde toca

        //  printf("Bytes 2 = %d", cant_bytes);
        if (cant_bytes == FALLO)
        {
    ...
    { // Caso 2 (Escritura en más de 1 bloque)
        ...
        for (int bl = primerBL + 1; bl < ultimoBL; bl++)
        {
            ...
            if (nbfisico == FALLO)
            {
                perror(RED "Error en mi_write_f Caso 2: No se ha podido traducir el bloque de inodo.\n" RESET);
                return FALLO;
                break;
            }
        ...
        if (nbfisico == FALLO)
        {
            perror(RED "Error en mi_write_f Caso 2 Fase 3: No se ha podido traducir el bloque de inodo.\n" RESET);
            return FALLO;
        }
        ...
        cant_bytes += (desp2 + 1);
        // printf("Bytes 5 = %d", cant_bytes);
    }

    // Actualizamos la información del inodo
    if (inodos.tamEnBytesLog < offset + cant_bytes)
    { // Si hemos escrito más allá del tamEnBytesLog, lo actualizamos como nuevo tamaño máximo. /////////////////PODRÍA ESTAR MAL //////////////////////////
...
{ // Leemos un fichero. buf_original debe estar inicializado a 0's
    int cant_bytes = 0;
    struct inodo inodos;
    ...
    if (offset >= inodos.tamEnBytesLog)
    {                   // Si ya se encuentra fuera del EOF (End Of File)
        cant_bytes = 0; // No podemos leer nada
        return cant_bytes;
    }

    if ((offset + nbytes) >= inodos.tamEnBytesLog)
    {                                           // Si pretende leer más allá de EOF
    ...
    if (nbfisico == FALLO)
    { // Si vale -1 no existe y hay que saltar el bloque
    ...
    // 2 Casos
    if (primerBL == ultimoBL)
    { // Caso 1 (Lectura de 1 solo bloque [ya lo tenemos leído de antes])
        ...
        { // Hemos leído bloque -> copiamos la info en buf_original
    ...
    { // Caso 2 (Lectura de más de 1 bloque)
        ...
        for (int bl = primerBL + 1; bl < ultimoBL; bl++)
        { /////////////////////////LA CONDICIÓN PODRÍA ESTAR MAL (no creo).  //Recorremos todos menos el último/////////////////////////
